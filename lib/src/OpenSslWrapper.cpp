...
BIO* OpenSslWrapper::BIO_new(const BIO_METHOD* type) const { return ::BIO_new(type); }
...
int OpenSslWrapper::PEM_write_bio_RSAPrivateKey(BIO* bp, RSA* x, const EVP_CIPHER* enc,
                                         unsigned char* kstr, int klen,
                                         pem_password_cb* cb, void* u) const {
    return ::PEM_write_bio_RSAPrivateKey(bp, x, enc, kstr, klen, cb, u);
}

EVP_PKEY* OpenSslWrapper::PEM_read_bio_PrivateKey(BIO* bp, EVP_PKEY** x,
                                           pem_password_cb* cb, void* u) const {
...
void OpenSslWrapper::RSA_get0_key(const RSA* r, const BIGNUM** n, const BIGNUM** e,
